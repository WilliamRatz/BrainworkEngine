...
double MathLib::Cos(const double& angle) {
	...
	double result = size + MathLib::fract(angle);
	result *= toDeg;
	result = 1 - MathLib::pow(result, 2) / MathLib::faculty(2) + MathLib::pow(result, 4) / MathLib::faculty(4) - MathLib::pow(result, 6) / MathLib::faculty(6) + MathLib::pow(result, 8) / MathLib::faculty(8);
	size -= 90;
	result *= MathLib::sign(size);
...
int  MathLib::sign(const int& value)
{
