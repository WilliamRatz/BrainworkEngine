...
void Texture::SetBuffer(VK_Buffer& p_vk_buffer)
{
	vk_buffer = &p_vk_buffer;
}
...
VkImageView Texture::createImageView(VkImage image, VkFormat format) {
	...
	if (vkCreateImageView(vk_buffer->renderer->vk_device->device, &viewInfo, nullptr, &imageView) != VK_SUCCESS) {
...
void Texture::CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
	...
	if (vkCreateImage(vk_buffer->renderer->vk_device->device, &imageInfo, nullptr, &image) != VK_SUCCESS) {
		throw std::runtime_error("failed to create image!");
	}

	VkMemoryRequirements memRequirements;
	vkGetImageMemoryRequirements(vk_buffer->renderer->vk_device->device, image, &memRequirements);
	...
	if (vkAllocateMemory(vk_buffer->renderer->vk_device->device, &allocInfo, nullptr, &imageMemory) != VK_SUCCESS) {
		throw std::runtime_error("failed to allocate image memory!");
	}

	vkBindImageMemory(vk_buffer->renderer->vk_device->device, image, imageMemory, 0);
}
